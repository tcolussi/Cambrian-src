...
#define IN_MOD_TMP		// The input parameter is temporary modified and restore to its original value before the routine terminates. This is often used for optimization to avoid duplicating the data, such as when parsing a string and inserting a temporary null-terminator withi the string.
