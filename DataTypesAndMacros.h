...
typedef UINT TIMESTAMP_MINUTES;	// tsm - Number of minutes since the application started
...
#define INOUT_F_UNCH_S	// The parameter is essentially OUT_F_UNCH, however the routine may return success without modifying the output parameter.  Of course, if the routine fails, the output parameter remains unchanged.  An example of this is parsing a number; if the string is empty, then the output parameter (storage for the number) is unchanged and the routine returns success.  If the string contains invalid data, the number remains unchanged and the routine returns failure or an error code.  Another example is an encryption routine which may return success without doing any encryption.
...
#define INOUT_ZZR		// The parameter is a combination of IN and OUT_ZZR
...
#define PA_CHILD		// The parameter is becoming child of the function (or object), and therefore the function (or object) will take care of deleting the allocated object when going out of scope.
