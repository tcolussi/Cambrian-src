...
#include <../src/gui/widgets/dlgchooser.hpp>
#include <../src/gui/widgets/overridecursor.hpp>
#include <../src/gui/widgets/compose.hpp>
#include <../src/gui/ui/getstringdialog.hpp>
...
//  connect(this, SIGNAL(balanceChanged()),        m_pHome, SLOT(onBalancesChanged()));
    ...
    // --------------------------------------------------------
    connect(this, SIGNAL(showContact(QString)),               OTX::It(), SLOT(mc_showcontact_slot(QString)));
    // --------------------------------------------------------
    connect(this, SIGNAL(showContactAndRefreshHome(QString)), m_pHome,            SLOT(onNeedToRefreshRecords()));
    connect(this, SIGNAL(showContactAndRefreshHome(QString)), OTX::It(), SLOT(mc_showcontact_slot(QString)));
    // --------------------------------------------------------
    connect(this, SIGNAL(balanceChanged()),                   OTX::It(), SLOT(onBalancesChanged()));
    connect(this, SIGNAL(balanceChanged()),                   m_pHome,            SLOT(onAccountDataDownloaded()));
    // --------------------------------------------------------
    connect(this, SIGNAL(recordDeletedBalanceChanged(bool)),  OTX::It(), SLOT(onBalancesChanged()));
    connect(this, SIGNAL(recordDeletedBalanceChanged(bool)),  m_pHome,            SLOT(onRecordDeleted(bool))); // bRefreshUserBar
    // NOTE: ALWAYS pass false to recordDeletedBalanceChanged, since OTX::onBalancesChanged refreshes
...
QString MTHomeDetail::FindAppropriateDepositAccount(OTRecord & recordmt)
{
    ...
    if (m_pHome)
        qstr_acct_id = OTX::It()->get_default_account_id();
    // -----------------------------------
    ...
    if (m_record)
    {
        ...
        if (recordmt.IsCash())
        {
            ...
            if (!qstr_acct_id.isEmpty())
            {
                ...
                if (!bSuccess)
                {
                    const int64_t lUsageCredits = OTX::HasUsageCredits(this,
