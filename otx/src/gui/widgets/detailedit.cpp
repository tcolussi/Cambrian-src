...
#include <../src/gui/widgets/detailedit.hpp>
#include <ui_detailedit.h>

#include <../src/gui/widgets/nymdetails.hpp>
#include <../src/gui/widgets/contactdetails.hpp>
#include <../src/gui/widgets/serverdetails.hpp>
#include <../src/gui/widgets/assetdetails.hpp>
#include <../src/gui/widgets/agreementdetails.hpp>
#include <../src/gui/widgets/corporationdetails.hpp>
#include <../src/gui/widgets/offerdetails.hpp>
#include <../src/gui/widgets/marketdetails.hpp>
#include <../src/gui/widgets/accountdetails.hpp>
#include <../src/gui/widgets/transportdetails.hpp>
#include <../src/gui/widgets/home.hpp>
...
//      theDlg.installEventFilter(this);
        ...
        //theDlg.setWindowFlags(Qt::Tool); // A hack so it will show the close button.
          ...
          //theDlg.setWindowFlags( Qt::Window | Qt::FramelessWindowHint);

         theDlg.setModal(true);
         theDlg.showMaximized();
          theDlg.exec();
    ...
    if (m_bFirstRun)
    {
        ...
        case MTDetailEdit::DetailEditTypeAccount:
            ...
            connect(m_pDetailPane,      SIGNAL(DefaultAccountChanged(QString, QString)),
                    OTX::It(), SLOT  (setDefaultAccount(QString, QString)));
            // -------------------------------------------
            connect(m_pDetailPane,      SIGNAL(SendFromAcct(QString)),
                    OTX::It(), SLOT  (mc_send_from_acct(QString)));
            // -------------------------------------------
            connect(m_pDetailPane,      SIGNAL(RequestToAcct(QString)),
                    OTX::It(), SLOT  (mc_request_to_acct(QString)));
            // -------------------------------------------
            break;
        ...
        connect(m_pDetailPane,      SIGNAL(NeedToUpdateMenu()),
                OTX::It(), SLOT  (onNeedToUpdateMenu()));
        // -------------------------------------------
        ...
        connect(m_pDetailPane,      SIGNAL(ShowAsset(QString)),
                OTX::It(), SLOT  (mc_show_asset_slot(QString)));
        // -------------------------------------------
        connect(m_pDetailPane,      SIGNAL(ShowNym(QString)),
                OTX::It(), SLOT  (mc_show_nym_slot(QString)));
        // -------------------------------------------
        connect(m_pDetailPane,      SIGNAL(ShowServer(QString)),
                OTX::It(), SLOT  (mc_show_server_slot(QString)));
        // -------------------------------------------
        connect(m_pDetailPane,      SIGNAL(ShowAccount(QString)),
                OTX::It(), SLOT  (mc_show_account_slot(QString)));
        // -------------------------------------------
        ...
        ui->labelCredential->setVisible(true);
        // ----------------------------------
        if (!m_bEnableAdd)
...
void MTDetailEdit::on_addButton_clicked(bool checked)
{

}

void MTDetailEdit::on_closeBTN_clicked()
{
    g_RCS_Operation=0; // CLOSE OPERATION
    g_RCS_nymId_Operation=-1; // no operation
    //g_RCS_nymId_Current= //get selected nym
    this->parentWidget()->close();
}

void MTDetailEdit::on_switchButton_clicked()
{   //TODO: Tie the global g_RCS_nymID_Operation every change of selection on credential screen
    g_RCS_Operation=2; //set option to Switch
   //g_RCS_nymId_Operation= //get selected nym
     g_RCS_nymId_Current=g_RCS_nymId_Operation;
}
