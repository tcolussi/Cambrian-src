...
#include <../src/gui/widgets/homedetail.hpp>
#include <../src/gui/widgets/overridecursor.hpp>
...
QWidget * MTHome::CreateUserBarWidget()
{
   ...
   //JORGE. Comment only next 2 lines
   ...
   // pUserBarWidget_layout->setContentsMargins(12, 3, 8, 10); // left top right bottom
    ...
    pUserBarWidget->setStyleSheet("QWidget{background-color:lightgreen;selection-background-color:#a0aac4;}");
    // -------------------------------------------
    ...
    QString qstr_acct_id = OTX::It()->get_default_account_id();
    // -------------------------------------------
    if (qstr_acct_id.isEmpty())
    {
        qstr_acct_name   = tr("(Default Account Isn't Set Yet)");
        // -----------------------------------
        qstr_acct_nym    = OTX::It()->get_default_nym_id();
        qstr_acct_server = OTX::It()->get_default_server_id();
        qstr_acct_asset  = OTX::It()->get_default_asset_id();
    }
    else
    {
        std::string str_acct_id     = qstr_acct_id.toStdString();
    ...
    // -------------------------------------------
    connect(buttonAccount, SIGNAL(clicked()), OTX::It(), SLOT(mc_show_account_manager_slot()));
    // ----------------------------------------------------------------
...
*/
   ...
   // pAccountLayout->addWidget(asset_type);
    ...
    // Jorge.   only showed buttons ----------------------------------------------------------------
   // currency_amount_label->setStyleSheet("QLabel { color : gray; }");
...
//    buttonRequest->setText(tr("Request"));
    ...
    buttonIdentities->setText(tr("Create NYM"));
    // ----------------------------------------------------------------
...
//  pButtonLayout->addWidget(buttonRequest);
    ...
    // JORGE.  CHANGE Qt::AlignRight to AlignLeft
    pUserBarWidget_layout->addLayout(pButtonLayout, 0, 1, 1,1, Qt::AlignLeft);
//  pUserBarWidget_layout->addWidget(currency_amount_label, 0, 1, 1,1, Qt::AlignRight);
    // -------------------------------------------
//  connect(buttonRequest,    SIGNAL(clicked()),  OTX::It(), SLOT(mc_requestfunds_slot()));
    connect(buttonSend,       SIGNAL(clicked()),  OTX::It(), SLOT(mc_sendfunds_slot()));
    connect(buttonCompose,    SIGNAL(clicked()),  OTX::It(), SLOT(mc_composemessage_slot()));
    connect(buttonIdentities, SIGNAL(clicked()),  OTX::It(), SLOT(mc_defaultnym_slot()));
    connect(buttonContacts,   SIGNAL(clicked()),  OTX::It(), SLOT(mc_addressbook_slot()));
