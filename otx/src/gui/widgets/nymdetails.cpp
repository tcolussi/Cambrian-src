...
#include <../src/gui/widgets/credentials.hpp>
#include <../src/gui/widgets/wizardaddnym.hpp>
...
void MTNymDetails::DeleteButtonClicked()
{
    if (!m_pOwner->m_qstrCurrentID.isEmpty())
    {
        ...
        if (reply == QMessageBox::Yes)
        {
            ...
            std::string deletedNymId=m_pOwner->m_qstrCurrentID.toStdString();
            if (bSuccess)
            {
                ...
                // ------------------------------------------------
                g_RCS_nymId_Current=m_pOwner->m_qstrCurrentID.toStdString(); // The current selected row
                g_RCS_nymId_Operation=deletedNymId;  // The nym related with operation
                g_RCS_Operation=3; // Delete Nym Operation
                wparentWidget->parentWidget()->close(); // close go to update Cambrian after delete nym in OT
            }
            else
...
// CUE_NYMDETAILS_BUTTON_ACTIONS
// ------------------------------------------------------

//virtual
void MTNymDetails::CloseButtonClicked()
{
...
void MTNymDetails::AddButtonClicked()
{
    ...
    theWizard.setWindowTitle(tr("Create Role & Credentials (Public and Private Key)"));

    if (QDialog::Accepted == theWizard.exec())
    {
        QString qstrName        = theWizard.field("Name")     .toString();
        int     nKeysizeIndex   = theWizard.field("Keysdsaize")  .toInt();
...
//                                 arg(qstrName).arg(tr("ID")).arg(qstrID));
        ...
        // Update the global array that insert the new nym and role in cambrian


       g_RCS_nymId_Current=qstrID.toStdString(); // The current selected row
       g_RCS_nymId_Operation=qstrID.toStdString();  // The nym related with operation
       g_RCS_Operation=1; // Add Nym Operation
        wparentWidget->parentWidget()->close();
        // close go to update Cambrian with the new nym
