...
//Global array to hold the status of credential screen
extern int g_RCS_Operation; //operation of credential screen 0=cancel 1=Add 2=switch 3=delete
extern std::string g_RCS_nymId_Current; // holds current selected nym
extern std::string g_RCS_nymId_Operation; // holds the last nymId related with a particular g_RCS_Operation

class OTX : public QWidget
{
    Q_OBJECT

private:
    /** Constructor & Destructor **/
    OTX(QWidget *parent = 0);
public:

    virtual ~OTX();

    static OTX * It(QWidget *parent = 0, bool bShuttingDown = false);
    ...
    void bootTray();
     ...
     void mc_show_nym(QString text, int &pRCS_Operation, std::string &pRCS_nymId_Current, std::string &pRCS_nymId_Operation);
...
public:
    /* Public Methods*/
    void RoleManagement(QString text);
