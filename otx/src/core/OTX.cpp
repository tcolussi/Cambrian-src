...
int g_RCS_Operation; //operation of credential screen 0=cancel 1=Add 2=switch 3=delete
std::string g_RCS_nymId_Current; // holds current selected nym
std::string g_RCS_nymId_Operation; // holds the last nymId related with a particular g_RCS_Operation
/**
 * Constructor & Destructor
 **/

//static
OTX * OTX::It(QWidget *parent/*=0*/, bool bShuttingDown/*=false*/)
{
    // NOTE: parent is only used the first time this function is called.
    // (And the last time.)
    //
    static QScopedPointer<OTX> pOTX(new OTX(parent));
  ...
  account_list_name(NULL)
{
     ...
     ** Init variables *
     **/
    // by default select the first nym in db
...
//            qDebug() << "Error loading DEFAULT ACCOUNT from SQL";
    ...
    mc_systrayIcon->setIcon(QIcon(":/icons/OTX"));
...
// New methods added by Central Services


void OTX::RoleManagement(QString text)
{
...
int64_t OTX::HasUsageCredits(      QWidget     * parent,
                                      ...
                                      const std::string & NYM_ID)
{
    ...
    if (strMessage.empty())
    {
        ...
        qstrErrorHeader = tr("OTX::HasUsageCredits: Error 'strMessage' is Empty!");
                                      ...
                                      QString   NYM_ID)
{
    ...
    return OTX::HasUsageCredits(parent, str_server, str_nym);
}
...
void OTX::SetupMainMenu()
{
    ...
    //App name
    mc_systrayMenu_headertext = new QAction(tr("OTX"), mc_systrayMenu);
 ...
 ****
 *
 * Remember that you'll need to define a close public function here in OTX, and the accompanying
 ...
 * 
 *  1) Startup Slot to connect to OTX Buttons (mc_classname_slot)
...
void OTX::mc_crypto_encrypt_slot()
{
    mc_encrypt_show_dialog(true, true);
}

void OTX::mc_crypto_sign_slot()
{
    mc_encrypt_show_dialog(false, true);
}

void OTX::mc_decrypt_show_dialog()
{
    // --------------------------------------------------
...
void OTX::mc_crypto_decrypt_slot()
{
    mc_decrypt_show_dialog();
}

void OTX::mc_crypto_verify_slot()
{
 //nothing for now. prolly forever.
}

void OTX::mc_encrypt_show_dialog(bool bEncrypt/*=true*/, bool bSign/*=true*/)
{
    // --------------------------------------------------
...
void OTX::mc_addressbook_show(QString text/*=QString("")*/)
{
...
void OTX::mc_addressbook_slot()
{
    mc_addressbook_show();
}

void OTX::mc_showcontact_slot(QString text)
{
...
//Nym manager "clicked"
void OTX::mc_defaultnym_slot()
{
    //The operator has requested to open the dialog to the "Nym Manager";
    mc_nymmanager_dialog();
}

void OTX::mc_show_nym(QString text, int &pRCS_Operation, std::string &pRCS_nymId_Current, std::string &pRCS_nymId_Operation)
{
    g_RCS_Operation=pRCS_Operation;
    g_RCS_nymId_Operation=pRCS_nymId_Operation;
    g_RCS_nymId_Current=pRCS_nymId_Current;

    mc_nymmanager_dialog(text);

    pRCS_Operation=g_RCS_Operation;
    pRCS_nymId_Operation=g_RCS_nymId_Operation;
    pRCS_nymId_Current=g_RCS_nymId_Current;
}
void OTX::mc_show_nym_slot(QString text)
{
    mc_nymmanager_dialog(text);

}

void OTX::mc_nymmanager_dialog(QString qstrPresetID/*=QString("")*/)
{
    ...
    // -------------------------------------
    nymswindow->setWindowTitle(tr("Society Pro Roles and Credentials"));
    // -------------------------------------
    if (bFoundPreset)
        nymswindow->SetPreSelected(qstrPresetID);
    // -------------------------------------
    nymswindow->dialog(MTDetailEdit::DetailEditTypeNym,true);
}



//Additional Nym Manager Functions
void OTX::setDefaultNym(QString nym_id, QString nym_name)
{
...
void OTX::mc_nymselection_triggered(QAction*action_triggered)
{
...
void OTX::onNeedToDownloadSingleAcct(QString qstrAcctID)
{
    ...
    if (bRetrievalAttempted)
    {
        if (!bRetrievalSucceeded)
            OTX::HasUsageCredits(NULL, acctSvrID, acctNymID);
        else
...
void OTX::onNeedToDownloadAccountData()
{
        ...
        if (!defaultNymID.empty() && !defaultServerId.empty())
        {
            ...
            if (!isReg)
            {
                ...
                if (!madeEasy.VerifyMessageSuccess(response))
                    OTX::HasUsageCredits(NULL, defaultServerId, defaultNymID);
        ...
        for (int32_t serverIndex = 0; serverIndex < serverCount; ++serverIndex)
        {
            ...
            for (int32_t nymIndex = 0; nymIndex < nymCount; ++nymIndex)
            {
                ...
                if (bRetrievalAttempted && !bRetrievalSucceeded)
                    OTX::HasUsageCredits(NULL, serverId, nymId);
            }
        }
        // ----------------------------------------------------------------
...
void OTX::mc_assetmanager_dialog(QString qstrPresetID/*=QString("")*/)
{
...
//Set Default asset
void OTX::setDefaultAsset(QString asset_id, QString asset_name)
{
...
void OTX::mc_assetselection_triggered(QAction*action_triggered)
{
...
void OTX::mc_defaultaccount_slot()
{
...
void OTX::mc_show_account_slot(QString text)
{
    mc_accountmanager_dialog(text);
}

void OTX::mc_show_account_manager_slot()
{
    mc_accountmanager_dialog();
}

void OTX::mc_accountmanager_dialog(QString qstrAcctID/*=QString("")*/)
{
    ...
    if (!accountswindow)
    {
        ...
        // it will call OTX's "onBalancesChanged" function.
        // (Which will trigger OTX's "balancesChanged" signal.)
        //
        connect(accountswindow, SIGNAL(balancesChanged()),
                this,           SLOT(onBalancesChanged()));

        // When OTX's signal "balancesChanged" is triggered,
...
void OTX::mc_accountselection_triggered(QAction*action_triggered)
{
...
//Set Default account
void OTX::setDefaultAccount(QString account_id, QString account_name)
{
...
void OTX::mc_show_asset_slot(QString text)
{
    mc_assetmanager_dialog(text);
}


void OTX::mc_show_server_slot(QString text)
{
...
void OTX::mc_defaultserver_slot()
{
    mc_servermanager_dialog();
}


void OTX::mc_servermanager_dialog(QString qstrPresetID/*=QString("")*/)
{
...
void OTX::setDefaultServer(QString server_id, QString server_name)
{
...
void OTX::mc_serverselection_triggered(QAction * action_triggered)
{
    //Check if the user wants to open the nym manager (or) select a different default nym
    QString action_triggered_string = QVariant(action_triggered->data()).toString();
...
void OTX::mc_requestfunds_slot()
{
    mc_requestfunds_show_dialog();
}

void OTX::mc_request_to_acct(QString qstrAcct)
{
    mc_requestfunds_show_dialog(qstrAcct);
}

void OTX::mc_requestfunds_show_dialog(QString qstrAcct/*=QString("")*/)
{
    // --------------------------------------------------
...
//          const bool bImported = OTAPI_Wrap::It()->Wallet_ImportPurse(strServerID, strAssetID, strPurseOwner, strInstrument);
            ...
            {
                const int64_t lUsageCredits = OTX::HasUsageCredits(NULL, strServerID, strPurseOwner);
...
void OTX::mc_sendfunds_slot()
{
    mc_sendfunds_show_dialog();
}

void OTX::mc_sendfunds_show_dialog(QString qstrAcct/*=QString("")*/)
{
    // --------------------------------------------------
...
void OTX::mc_send_from_acct(QString qstrAcct)
{
...
void OTX::mc_composemessage_slot()
{
    mc_composemessage_show_dialog();
}

void OTX::mc_composemessage_show_dialog()
{
    // --------------------------------------------------
    MTCompose * compose_window = new MTCompose;
    compose_window->setAttribute(Qt::WA_DeleteOnClose);
    // --------------------------------------------------
    // If OTX has a default Nym set, we use that for the Sender.
...
void OTX::mc_agreement_slot()
{
    mc_agreement_dialog();
}

void OTX::mc_agreement_dialog()
{
    // -------------------------------------
    if (!agreement_window)
...
void OTX::mc_market_slot()
{
    mc_market_dialog();
}

void OTX::mc_market_dialog()
{
    if (!market_window)
    {
...
void OTX::mc_corporation_slot()
{
    mc_corporation_dialog();
}

void OTX::mc_corporation_dialog()
{
    // -------------------------------------
...
void OTX::mc_transport_slot()
{
    mc_transport_dialog();
}

// Same, except choose a specific one when opening.
void OTX::mc_showtransport_slot(QString text)
{
    mc_transport_dialog(text);
}

void OTX::mc_transport_dialog(QString qstrPresetID/*=QString("")*/)
{
    if (!transport_window)
...
/**
  ...
  **/
void OTX::mc_bitcoin_connect_slot()
{
...
/**
  * (Bitcoin ->) Pools
  **/
void OTX::mc_bitcoin_pools_slot()
{
...
/**
  ...
  **/
void OTX::mc_bitcoin_transactions_slot()
{
    if(!bitcoinTxWindow)
...
/**
  * (Bitcoin ->) Receive
  **/
void OTX::mc_bitcoin_receive_slot()
{
