...
private:
    ...
    // For example, the method ID (if one is available.) If you use OTX
    ...
    int32_t       m_nMethodID;            // A Nym in OTX might have 2 Bitmessage addresses, each used on different BM nodes with different connection strings. The Method ID is used to lookup the connection string.
